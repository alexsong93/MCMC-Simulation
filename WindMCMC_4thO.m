...
quadgrams = cellstr(num2str( [x(1:end-4);x(2:end-3);x(3:end-2);x(4:end-1)]' ));

%# all possible combinations of two symbols
[X,Y,Z,Z1] = ndgrid(1:M,1:M,1:M,1:M);
xyzz1 = cellstr(num2str([X(:),Y(:),Z(:),Z1(:)]));

%# map bigrams to numbers starting from 1
[g,gn] = grp2idx([xyzz1;quadgrams]);
s1 = g(M*M*M*M+1:end);
...
%# transition matrix
P = full(sparse(s1,s2,1,M*M*M*M,M));
temp = P;
% 
...
%      chosen_value=wind_values(randi([1,numel(wind_values)],1));
     ...
     %r = cell2mat(xyzz1(start_row));
     %start_row = (numstates.^3).*(next_state-1) + (numstates.^2).*(next_state-1) + (numstates).*(next_state-1) + (r(4)-48);
