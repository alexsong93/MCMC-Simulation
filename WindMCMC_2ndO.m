...
simul_len=157968*2; % this is the length of the simulated wind power data
...
%start_wind=sum(Wind_10min(1:24:end))/numel(Wind_10min(1:24:end));
%Wind_10min=[0 1 22 35 2 18 17 14]; % this is test data
...
bzigrams = cellstr(num2str( [x(1:end-2);x(2:end-1)]' ));
...
%# transition matrix
P = full(sparse(s1,s2,1,M*M,M));
temp = P;
% 

% [P, xy] = getTransitionMatrix(x,2);
% temp = P;
...
%      chosen_value=wind_values(randi([1,numel(wind_values)],1));
     ...
     %r = cell2mat(xy(start_row));
     %start_row = numstates.*(next_state-1) + (xy{start_row}(end)-48);
     if(numstates < 10)
        start_row = (numstates).*(next_state-1) + (xy{start_row}(4)-48);
     else
        if(xy{start_row}(5)+0 == 32)
            start_row = (numstates).*(next_state-1) + (xy{start_row}(6)-48);
        else
            start_row = (numstates).*(next_state-1) + str2double(xy{start_row}(5))*10 +(xy{start_row}(6)-48);
        end
     end
